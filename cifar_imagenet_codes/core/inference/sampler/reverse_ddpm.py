...
def reverse_ddpm_naive(x_init, betas, small_sigma, clip_denoise, rescale_timesteps, steps_type='linear', clip_sigma_idx=0, clip_pixel=2,
                       eps_model=None, d_model=None, sample_steps=None, shift1=False):
    ...
    logging.info("reverse_ddpm_naive with {}, small_sigma={}, clip_denoise={}, rescale_timesteps={}, "
                 "sample_steps={}, steps_type={}, clip_sigma_idx={}, clip_pixel={}, shift1={}"
                 .format("eps_model" if eps_model is not None else "d_model",
                         small_sigma, clip_denoise, rescale_timesteps, sample_steps, steps_type, clip_sigma_idx, clip_pixel, shift1))
...
@ torch.no_grad()
def reverse_ddpm_ms_eps(x_init, betas, rescale_timesteps, steps_type='linear', clip_sigma_idx=0, clip_pixel=2, eps_model=None,
                        ms_eps=None, sample_steps=None, shift1=False):
    ...
    logging.info("reverse_ddpm_ms_eps with eps_model, rescale_timesteps={}, sample_steps={}, steps_type={}, clip_sigma_idx={}, clip_pixel={}, shift1={}"
                 .format(rescale_timesteps, sample_steps, steps_type, clip_sigma_idx, clip_pixel, shift1))
    ...
    for s, r in list(zip([0] + ns, ns))[::-1]:
        ...
        x_0_pred, eps_pred = _x_0_pred(x, r, cum_alphas, rescale_timesteps, eps_model=eps_model, shift1=shift1)
